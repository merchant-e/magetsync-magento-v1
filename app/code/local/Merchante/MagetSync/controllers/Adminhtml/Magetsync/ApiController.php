        ...
        try {
                ...
                Mage::getSingleton('adminhtml/session')
                    ->addError(
                        Mage::helper('magetsync')->__(
                            "You must fill in your Secret Token under System > Configuration > MagetSync"
                        )
                    );
                $this->_redirect('adminhtml/system_config/edit/section/magetsync_section/');
            ...
            if ($rVerification['success']) {
                ...
                if ($shop_name == $rVerification['shop_name'] && $rVerification['authorized'] == true) {
                    Mage::getSingleton('adminhtml/session')
                        ->addSuccess(
                            Mage::helper('magetsync')->__('This shop has been already authorized')
                        );
                    Mage::getSingleton('adminhtml/session')
                        ->settestData(false);
                    $this->_redirect('adminhtml/system_config/edit/section/magetsync_section/');
                ...
                if ($rVerification['success'] == true &&
                    ($rVerification['shop_name'] == '' || $shop_name != $rVerification['shop_name'])
                ) {
                    ...
                    if ($bverifier == '') {
                        ...
                        if ($result['status'] != false) {
                            ...
                            $data = array(
                                'TokenSecret'       => $btokenSecret,
                                'AccessToken'       => $acc_token['oauth_token'],
                    ...
                    Mage::log(
                        "Path: " . print_r(getcwd() . "/CACerts/cacert.pem", true), null,
                        'magetsync_certpath.log'
                    );
