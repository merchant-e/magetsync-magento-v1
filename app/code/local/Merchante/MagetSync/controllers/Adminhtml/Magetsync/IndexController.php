            ...
            try {
                ...
                foreach ($listings as $listing) {
                    ...
                    if (($readyToSync && $listing->getSync() == Merchante_MagetSync_Model_Listing::STATE_INQUEUE)
                        ...
                        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('magetsync')->__("Product with id:" . $listing->getIdproduct() . " can't be queued."));
                    }
                }
                if ($cnt) {
                    Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('magetsync')->__($cnt . " products were queued."));
                }
                $this->_redirect('adminhtml/magetsync_index/index');
                return;
            } catch (Exception $e) {
                ...
                try {
                    ...
                    foreach ($listings as $value) {
                        ...
                        if ($hasError == true) {
                            ...
                            if (!$syncStatus) {
                                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('magetsync')->__('Successfully saved'));
                            } else {
                                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('magetsync')->__('Successfully synchronized'));
                            }
            ...
            if(count($data['listingids']) > 3){
                ...
                foreach ($listings as $listing) {
                    ...
                    if (count($dataPro['media_gallery']['images']) > 0) {
                        ...
                        $resultIndex = $this->__searchForFile($imageUrl, $newImages);
