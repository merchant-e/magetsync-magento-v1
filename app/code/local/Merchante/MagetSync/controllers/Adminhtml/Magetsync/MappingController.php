...
error_reporting(E_ALL ^ E_NOTICE);
            ...
            foreach($idsListings as $listing)
            {
                ...
                if($resultListing['status'])
                {
                    $collectionProduct = Mage::getModel('catalog/product')->getCollection()
                        ->addAttributeToSelect(array('sku', 'name'), 'inner')
    ...
    {
        $json = array('message'=>$e->getMessage(),"status"=>false);
        $data = json_encode($json,true);
        echo $data;
    }
        }
        ...
        public function loadProductsAction()
        {
            try {
              $productsCollection = Mage::getModel('catalog/product')->getCollection()
                    ...
                    ->addAttributeToFilter('type_id', array('in' => array('simple','configurable')))->addAttributeToFilter(array(array('attribute'=>'synchronizedEtsy','null'=>true),array('attribute'=>'synchronizedEtsy','eq'=>'0'),array('attribute'=>'synchronizedEtsy','eq'=>'')),'','left')->load();//'synchronizedEtsy', array('null'=>true),'left')->addAttributeToFilter('synchronizedEtsy', array('eq'=>false));
                $rows = '';
                ...
                if (count($productsCollection) > 0) {
                    $json = array('data' => $rows, "status" => true);
                }else{
                    $json = array('data'=>'There aren\'t products available',"status"=>false);
                }
                $result = json_encode($json, true);
                echo $result;
            }catch (Exception $e)
            {
                $json = array('data'=>$e->getMessage(),"status"=>false);
                $result = json_encode($json, true);
                echo $result;
            }
        ...
        public function startMappingAction()
        {
            ...
            if($result['success'])
            {
                ...
                $json = array('message'=>$result['message'],"status"=>false);
            }
            $data = json_encode($json,true);
            echo $data;
            ...
            try {
                ...
                if ($query){
                    if( $query[0]['sync'] != Merchante_MagetSync_Model_Listing::STATE_SYNCED) {
                        ...
                        $json = array('message'=>'You can not delete the listing because is already synchronized',"status"=>false);
                        $data = json_encode($json,true);
                        echo $data;
                    }
            }
    else{
            $json = array('message'=>'Listing doesn\'t exist ',"status"=>false);
            $data = json_encode($json,true);
            echo $data;
        }
}catch(Exception $e)
        ...
        public function setMappingAction()
        {
            try {
                ...
                if ($query) {
                    ...
                    $queryListing = Mage::getSingleton('core/resource')->getConnection('core_read')->fetchAll($queryListing);
                    $resultListing['status'] = true;
                    if(!$queryListing) {
                    ...
                    if($resultListing['status'])
                    {
                        ...
                        $queryProduct = Mage::getSingleton('core/resource')->getConnection('core_read')->fetchAll($queryProductAux);
                        if ($queryProduct) {
